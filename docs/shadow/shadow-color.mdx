...
:::info
This property will only work on Android API 28 and above. For similar functionality on lower Android APIs, use the [elevation property](/docs/elevation).
:::
    ...
    <tbody>
      {Object.entries(colorList).map(([key, value]) => (
        <React.Fragment key={key}>
          <tr>
            <td>{`shadow.color_${key}`}</td>
            <td>
              <code>{`{ shadowColor: '${value}' }`}</code>
            </td>
            <td>
              <div
                style={{
                  ...
                  borderRadius: "50%",
                  boxShadow: `0 0 8px 4px ${value}80`, // 80 is the hex value for 50% opacity
                }}
              ></div>
            </td>
          </tr>
...
| ------------------------ | ------------------------------ |
| shadow.color_(customColor) | `{ shadowColor: customColor }` |
